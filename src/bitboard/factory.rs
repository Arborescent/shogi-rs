use super::StandardBitboard;
use crate::color::Color;
use crate::piece_type::PieceType;
use crate::square::StandardSquare;
use bitintr::*;

// Type aliases for standard 9x9 board factory (current implementation)
type Bb = StandardBitboard;
type Sq = StandardSquare;

macro_rules! BitboardOr {
    ($lhs: expr, $rhs: expr) => {
        Bb::from_parts($lhs.p[0] | $rhs.p[0], $lhs.p[1] | $rhs.p[1])
    };
}

/// Helper function to create a bitboard with a single square set (for internal use)
#[inline(always)]
fn square_bb(sq: Sq) -> Bb {
    Bb::from_square(sq)
}

/// Creates various bitboard instances for standard 9×9 Shogi.
///
...
///
/// // init() should be called before other method calls.
/// Factory::init();
...
impl Factory {
    ...
    #[inline(always)]
    pub fn attacks_from(pt: PieceType, c: Color, sq: Sq) -> Bb {
    ...
    #[inline(always)]
    pub fn rook_attack(sq: Sq, occupied: &Bb) -> Bb {
        unsafe {
    ...
    #[inline(always)]
    pub fn bishop_attack(sq: Sq, occupied: &Bb) -> Bb {
        unsafe {
    ...
    #[inline(always)]
    pub fn lance_attack(c: Color, sq: Sq, occupied: &Bb) -> Bb {
        unsafe {
    ...
    #[inline(always)]
    pub fn promote_zone(c: Color) -> Bb {
        match c {
    ...
    #[inline(always)]
    pub fn between(sq1: Sq, sq2: Sq) -> Bb {
        unsafe { BETWEEN_BB[sq1.index()][sq2.index()] }
    }
}

const EMPTY_BB: Bb = Bb::from_parts(0, 0);
const FULL_BB: Bb = Bb::from_parts(0x7fff_ffff_ffff_ffff, 0x0000_0000_0003_ffff);

const FILE1_BB: Bb = Bb::from_parts(0x1FF, 0);
const FILE2_BB: Bb = Bb::from_parts(0x1FF << 9, 0);
const FILE3_BB: Bb = Bb::from_parts(0x1FF << (9 * 2), 0);
const FILE4_BB: Bb = Bb::from_parts(0x1FF << (9 * 3), 0);
const FILE5_BB: Bb = Bb::from_parts(0x1FF << (9 * 4), 0);
const FILE6_BB: Bb = Bb::from_parts(0x1FF << (9 * 5), 0);
const FILE7_BB: Bb = Bb::from_parts(0x1FF << (9 * 6), 0);
const FILE8_BB: Bb = Bb::from_parts(0, 0x1FF);
const FILE9_BB: Bb = Bb::from_parts(0, 0x1FF << 9);
const FILE_BB: [Bb; 9] = [
    FILE1_BB, FILE2_BB, FILE3_BB, FILE4_BB, FILE5_BB, FILE6_BB, FILE7_BB, FILE8_BB, FILE9_BB,
];

const RANK1_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201, 0x201);
const RANK2_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 1, 0x201 << 1);
const RANK3_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 2, 0x201 << 2);
const RANK4_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 3, 0x201 << 3);
const RANK5_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 4, 0x201 << 4);
const RANK6_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 5, 0x201 << 5);
const RANK7_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 6, 0x201 << 6);
const RANK8_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 7, 0x201 << 7);
const RANK9_BB: Bb = Bb::from_parts(0x0040_2010_0804_0201 << 8, 0x201 << 8);
const RANK_BB: [Bb; 9] = [
    RANK1_BB, RANK2_BB, RANK3_BB, RANK4_BB, RANK5_BB, RANK6_BB, RANK7_BB, RANK8_BB, RANK9_BB,
];

const IN_FRONT_BLACK_RANK1_BB: Bb = EMPTY_BB;
const IN_FRONT_BLACK_RANK2_BB: Bb = RANK1_BB;
const IN_FRONT_BLACK_RANK3_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK2_BB, RANK2_BB);
const IN_FRONT_BLACK_RANK4_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK3_BB, RANK3_BB);
const IN_FRONT_BLACK_RANK5_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK4_BB, RANK4_BB);
const IN_FRONT_BLACK_RANK6_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK5_BB, RANK5_BB);
const IN_FRONT_BLACK_RANK7_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK6_BB, RANK6_BB);
const IN_FRONT_BLACK_RANK8_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK7_BB, RANK7_BB);
const IN_FRONT_BLACK_RANK9_BB: Bb = BitboardOr!(IN_FRONT_BLACK_RANK8_BB, RANK8_BB);

const IN_FRONT_WHITE_RANK9_BB: Bb = EMPTY_BB;
const IN_FRONT_WHITE_RANK8_BB: Bb = RANK9_BB;
const IN_FRONT_WHITE_RANK7_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK8_BB, RANK8_BB);
const IN_FRONT_WHITE_RANK6_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK7_BB, RANK7_BB);
const IN_FRONT_WHITE_RANK5_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK6_BB, RANK6_BB);
const IN_FRONT_WHITE_RANK4_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK5_BB, RANK5_BB);
const IN_FRONT_WHITE_RANK3_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK4_BB, RANK4_BB);
const IN_FRONT_WHITE_RANK2_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK3_BB, RANK3_BB);
const IN_FRONT_WHITE_RANK1_BB: Bb = BitboardOr!(IN_FRONT_WHITE_RANK2_BB, RANK2_BB);

const IN_FRONT_BB: [[Bb; 9]; 2] = [
    [
...
static mut ROOK_BLOCK_MASK: [Bb; 81] = [Bb::from_parts(0, 0); 81];
static mut ROOK_ATTACK_INDEX: [usize; 81] = [0; 81];
static mut ROOK_ATTACK_BB: [Bb; 495_616] = [Bb::from_parts(0, 0); 495_616];
...
static mut BISHOP_BLOCK_MASK: [Bb; 81] = [Bb::from_parts(0, 0); 81];
static mut BISHOP_ATTACK_INDEX: [usize; 81] = [0; 81];
static mut BISHOP_ATTACK_BB: [Bb; 20224] = [Bb::from_parts(0, 0); 20224];
...
static mut LANCE_ATTACK_BB: [[[Bb; 128]; 81]; 2] = [[[Bb::from_parts(0, 0); 128]; 81]; 2];
static mut ATTACK_BB: [[[Bb; 81]; 2]; 14] = [[[Bb::from_parts(0, 0); 81]; 2]; 14];

static mut BETWEEN_BB: [[Bb; 81]; 81] = [[Bb::from_parts(0, 0); 81]; 81];

#[inline(always)]
fn index_to_occupied(index: usize, bits: usize, mask: &Bb) -> Bb {
    let mut bb = Bb::empty();
...
#[inline(always)]
fn occupied_to_index(occupied: &Bb, mask: &Bb) -> usize {
...
fn calc_beam_attack(piece_sq: Sq, dirs: &[(i8, i8)], occupied: &Bb) -> Bb {
    let mut bb = Bb::empty();
    for dir in dirs {
