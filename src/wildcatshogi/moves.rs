//! Move representation for Wild Cat Shogi.
...
/// Represents a move in Wild Cat Shogi.
...
/// Helper: Convert file index to Japanese numeral (1-3 for Wild Cat Shogi).
...
fn piece_type_to_kanji(pt: PieceType) -> &'static str {
    match pt {
        ...
        PieceType::Gold => "金",
        PieceType::Pawn => "歩",
        // These should not appear in Wild Cat Shogi
        _ => "?",
    }
}
...
mod tests {
    ...
    fn from_sfen_invalid() {
        ...
        assert!(Move::from_sfen("4a1a").is_none()); // Invalid file (only 1-3)
        assert!(Move::from_sfen("1f1a").is_none()); // Invalid rank (only a-e)
        assert!(Move::from_sfen("G*2b").is_none()); // Gold not valid hand piece
        assert!(Move::from_sfen("N*3c").is_none()); // Knight not valid
        assert!(Move::from_sfen("L*3c").is_none()); // Lance not valid
    }
    ...
    fn to_japanese() {
        ...
        assert_eq!("☖３四飛打", drop.to_japanese(PieceType::Rook, Color::White));
    }
